...
//	 		"}";
  ...
  public void getuser() {
	  ...
	  Response response =RestAssured.given().contentType(ContentType.JSON)
			  .when().pathParam("username", "Priya")// passing path param
	...
	// Assertion
      ...
      response.then().body("username", equalTo("Priya"));
      response.then().body("firstName", equalTo("Priya"));
      response.then().body("lastName", equalTo("Singh"));
      response.then().body("email", equalTo("priya.singh@gmail.com"));
  ...
  public void deleteUser() throws IOException {
      Response response = 
          ...
          .pathParam("username", "Priya") // Add path parameter
          .when().delete(ROOT_URI + "/{username}"); // Send POST request

      // Assertion
      response.then().body("code", equalTo(200));
      response.then().body("message", equalTo("Priya"));
  }
